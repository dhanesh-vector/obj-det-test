...
#SBATCH --output=/fs01/home/dhaneshr/code/obj-det-test/slurm-script/logs/yoloe_tune_v4_%A_%a.out
#SBATCH --error=/fs01/home/dhaneshr/code/obj-det-test/slurm-script/logs/yoloe_tune_v4_%A_%a.err

# ─── Config ───────────────────────────────────────────────────────────────────
PROJECT_DIR="/fs01/home/dhaneshr/code/obj-det-test"
CONFIG_FILE="${1:-train/config/default.yaml}"
STUDY_NAME="yoloe_hparam_search_v4"
N_TRIALS=15          # trials per worker  (total = N_TRIALS × array_size = 300)
EPOCHS=50            # max epochs per trial
EARLY_STOP=15        # EMA SliceAP@50 patience (reduced from v3's 25 — EMA is
                     # already smoothed so a shorter window is safe)
...
CKPT_MIN_EMA=0.5     # only save checkpoints for trials that reach this EMA SliceAP@50
CKPT_DIR="${PROJECT_DIR}/checkpoints/hparam_v4"
...
python train/tune_hyperparams_v4.py \
    --config                    "$CONFIG_FILE" \
