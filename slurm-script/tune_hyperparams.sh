...
#SBATCH --array=0-11                 # 12 parallel workers  →  12 × 10 = 120 trials total
#SBATCH --output=/fs01/home/dhaneshr/code/obj-det-test/slurm-script/logs/yoloe_tune_%A_%a.out
#SBATCH --error=/fs01/home/dhaneshr/code/obj-det-test/slurm-script/logs/yoloe_tune_%A_%a.err

# ─── Config ───────────────────────────────────────────────────────────────────
PROJECT_DIR="/fs01/home/dhaneshr/code/obj-det-test"
CONFIG_FILE="${1:-train/config/default.yaml}"   # pass as first arg or use default
STUDY_NAME="yoloe_hparam_search_v2"
N_TRIALS=10          # trials per worker  (total = N_TRIALS × n_array_tasks = 120)
...
# Stagger workers slightly so they don't all hit the SQLite DB simultaneously
# on first-time study creation.  Task 0 gets 0 s, task 7 gets 7 s.
JITTER="${SLURM_ARRAY_TASK_ID}"

python train/tune_hyperparams.py \
    --config                    "$CONFIG_FILE" \
